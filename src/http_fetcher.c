...
static void _closesocket(socketCtx* ctx) {
	 ...
	 */
int http_fetch(const char *url_tmp, char **contentType, char **fileBuf)
	{
	fd_set rfds;
	struct timeval tv;
...
/*	while(!found &&
  ...
  */  do
	...
	/*
	 * Parse out the oontent-type.
	 * Note that some servers use different capitalization
	 */
	charIndex = strstr(headerBuf, "Content-Type:");
	if(charIndex == NULL)
		charIndex = strstr(headerBuf, "Content-type:");

	if (charIndex) {
		char *p = strstr(charIndex, "\r\n");

		*p = '\0';
		*contentType = strdup(charIndex + strlen("Content-type:"));

		for (p = *contentType; *p && *p == ' '; p++);
		memmove(*contentType, p, strlen(p) + 1);
		for (p = *contentType + strlen(*contentType) - 1; p > *contentType && *p == ' '; *p = '\0', p--);
	}
...
int _http_read_header(socketCtx* ctx, int sock, char *headerPtr)
	...
	headerPtr -= 2;		/* Snip the trailing LF's */
	*headerPtr = '\0';
	return bytesRead;
	}
